...
| KGMA/KGG → MP3 | ✅ (KGMA) | ✅ (KGMA+KGG) | 支持 KGMA v3 / slot 1 及 Windows 酷狗客户端 KGG v5 |
...
### 转换 KGMA 与 KGG
 ...
 1. 打开“导入”→“选择文件夹并转换 KGMA/KGG”。
 2. 选择包含 `.kgm`/`.kgma`/`.kgg` 的目录；应用会递归查找子目录。
 3. 每首歌先解密到临时文件：
    - **KGMA**：离线读取内嵌密钥解密；
    - **KGG**：自动读取本机 Windows 酷狗客户端数据库（`%APPDATA%\KuGou8\KGMusicV3.db`）中的密钥并解密；
    - 原始载荷已经是 MP3 时，直接无损复制，不重复编码；
    - FLAC/Ogg 等载荷通过 FFmpeg 转为 `320 kbps` MP3。
 4. MP3 成功写入 Resonance 管理目录后即完成转换；源 KGMA/KGG 始终保留，不会被应用删除。
 5. 普通系统扫描同样不会修改或删除原始文件。
 
 当前转换器支持 **KGMA v3 / slot 1** 及 **KGG v5**。其他版本会明确提示“不支持”，不会修改源文件。
